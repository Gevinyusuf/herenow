...
async def list_events(
    ...
    try:
        ...
        query = supabase.table("events_v1").select(
            "id, title, description, cover_image_url, start_at, end_at, timezone, location_info, visibility, ticket_config"
        ).eq("visibility", "public")

        if category:
            query = query.eq("category", category)

        query = query.order("start_at", desc=True).range((page-1)*limit, page*limit-1)
        result = query.execute()

        events = []
        for event in result.data:
            location_info = event.get("location_info", {}) or {}
            location_display = location_info.get("address", "TBD") if location_info.get("type") == "offline" else "Online Event"

            ticket_config = event.get("ticket_config", {}) or {}
            tickets = ticket_config.get("tickets", [])
            price = tickets[0].get("price", 0) if tickets else 0
        ...
        logger.error(f"list events failed: {str(e)}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"list events failed: {str(e)}"
        )
...
async def search_events(
    ...
    try:
        ...
        search_term = f"%{q}%"
        query = supabase.table("events_v1").select(
            "id, title, description, cover_image_url, start_at, end_at, location_info, ticket_config, category"
        ).eq("visibility", "public").ilike("title", search_term)

        if category:
            query = query.eq("category", category)

        result = query.order("start_at", desc=True).range((page-1)*limit, page*limit-1).execute()
        ...
        logger.error(f"search events failed: {str(e)}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"search events failed: {str(e)}"
        )
