...
async def upload_event_resource(
    ...
    try:
        ...
        if file_url:
            ...
            final_url = r2_client.get_file_url(r2_key) if hasattr(r2_client, 'get_file_url') else f"https://{r2_client.account_id}.r2.cloudflarestorage.com/{r2_client.bucket_name}/{r2_key}"
        else:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
        ...
        resource_data = {
            "event_id": event_id,
            "file_name": file_name or (file.filename if file else "unknown"),
            "file_size": len(file_content) if file else 0,
            "file_type": file.content_type if file else "application/octet-stream",
            "file_url": final_url,
            "r2_key": r2_key,
