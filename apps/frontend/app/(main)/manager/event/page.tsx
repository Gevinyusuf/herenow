...
import { getEvent, getEventRegistrations, getEventGuests, updateEvent, getEventResources, deleteEventResource, exportEventGuests, getGalleryPhotos, toggleGalleryPhotoLike, deleteGalleryPhoto, getEventRecap, saveEventRecap } from '@/lib/api/client';
...
function EventManagerPageContent() {
  ...
  const [eventRecap, setEventRecap] = useState({ title: '', content: '' });
  const [showAIModal, setShowAIModal] = useState(false);
  const [showPhotoModal, setShowPhotoModal] = useState(false);
  const [showResourceModal, setShowResourceModal] = useState(false);
  const recapEditorRef = useRef<HTMLDivElement>(null);
  ...
  useEffect(() => {
    const fetchEventData = async () => {
      ...
      try {
        ...
        try {
          ...
          if (photosData && Array.isArray(photosData)) {
            setGalleryPhotos(photosData.map((p: any) => ({
              id: p.id,
              image_url: p.url || p.image_url, // 兼容 url 和 image_url
        ...
        // 获取活动回顾
        try {
          const recapResponse = await getEventRecap(eventId);
          if (recapResponse && recapResponse.length > 0) {
            const recap = recapResponse[0];
            setEventRecap({
              title: recap.title || '',
              content: recap.content_data?.content || ''
            });
            if (recapEditorRef.current) {
              recapEditorRef.current.innerHTML = recap.content_data?.content || '';
            }
          }
        } catch (error) {
  ...
  const handleSaveRecap = async () => {
    if (!eventId || !recapEditorRef.current) return;

    try {
      setIsSaving(true);
      const content = recapEditorRef.current.innerHTML;
      await saveEventRecap(eventId, eventRecap.title || 'Event Recap', content);
      showToast('Recap saved successfully!');
      ...
      showToast('Failed to save recap');
    } finally {
      setIsSaving(false);
    }
  };
          ...
          {activeTab === 'content' && (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              <section className="lg:col-span-2 space-y-8">
                ...
                <div className="glass-card rounded-3xl p-6 shadow-herenow">
                  <div className="flex justify-between items-center mb-4">
                    <h2 className="text-2xl font-bold text-slate-900 font-brand">Event Recap</h2>
                    <button
                      onClick={handleSaveRecap}
                      disabled={isSaving}
                      className="text-sm font-semibold text-white px-4 py-1.5 rounded-lg hover:opacity-90 transition disabled:opacity-50 flex items-center gap-2"
                      style={{ backgroundColor: 'var(--color-herenow-orange)' }}
                    >
                  ...
                  <input
                    type="text"
                    value={eventRecap.title}
                    onChange={(e) => setEventRecap({ ...eventRecap, title: e.target.value })}
                    placeholder="Recap Title (e.g., 'A Night to Remember')"
                    className="w-full px-4 py-2 mb-3 border border-slate-200 rounded-xl focus:outline-none focus:border-orange-300 text-lg font-semibold"
                  />
                  <div className="flex items-center gap-1 mb-3 pb-3 border-b border-slate-200 flex-wrap">
                    <button onClick={() => execCmd('formatBlock', 'H1')} className="toolbar-btn font-extrabold text-sm" title="Heading 1">H1</button>
                    <button onClick={() => execCmd('formatBlock', 'H2')} className="toolbar-btn font-bold text-sm" title="Heading 2">H2</button>
                  ...
                  <div
                    ...
                    contentEditable
                    suppressContentEditableWarning
                    className="editor-content w-full bg-slate-50 rounded-xl p-4 text-slate-700 border border-slate-200 focus:outline-none focus:border-orange-300 min-h-[200px]"
                    placeholder="Write your event recap here..."
                    onInput={(e) => {
                      if (e.currentTarget) {
                        setEventRecap({ ...eventRecap, content: e.currentTarget.innerHTML });
                      }
                    }}
                  />
                </div>
