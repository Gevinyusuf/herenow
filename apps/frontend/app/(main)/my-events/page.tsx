...
export default function MyEventsPage() {
  ...
  const loadEvents = async () => {
    try {
      ...
      if (data.events) {
        ...
        } else if (data.events.past) {
          eventsArray = data.events.past;
        } else if (data.events.upcoming) {
          eventsArray = [...data.events.upcoming, ...data.events.past || []];
        }
      }
  ...
  const isEventEnded = (event: Event) => {
    ...
    const now = new Date();
    if (eventEnd.getTime() < now.getTime()) return true;
    return false;
  };
                ...
                <div className="p-6">
                  ...
                  {event.is_created && (
                    <div className="mt-4 pt-4 border-t border-slate-100 flex gap-2">
                      <Link
                        href={`/manager/event?id=${event.id}`}
                        className="flex-1 px-4 py-2 bg-[#FF6B3D] text-white text-center rounded-lg hover:bg-[#FF855F] transition-colors text-sm font-medium"
                      >
                        Manage Event
                      </Link>
                      <Link
