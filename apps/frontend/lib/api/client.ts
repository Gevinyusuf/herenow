 ...
 * 获取公开活动列表
 */
export async function getPublicEvents(page: number = 1, limit: number = 20, category?: string) {
  try {
    ...
    console.error('❌ getPublicEvents 失败:', error);
    throw error;
  }
}
 ...
 * 搜索活动
 */
export async function searchEvents(query: string, page: number = 1, limit: number = 20, category?: string) {
  try {
    let url = `/api/v1/events/search?q=${encodeURIComponent(query)}&page=${page}&limit=${limit}`;
    if (category) {
      url += `&category=${encodeURIComponent(category)}`;
    }
    const response = await fetch(url);
    ...
    console.error('❌ searchEvents 失败:', error);
    throw error;
  }
}
...
export async function uploadEventResource(
  ...
  try {
    ...
    if (typeof fileOrUrl === 'string') {
      ...
      body.append('file_url', fileOrUrl);
      if (fileName) body.append('file_name', fileName);
      body.append('require_registration', requireRegistration.toString());
    } else {
      // 文件上传模式
...
export async function getEventRecap(eventId: string) {
  try {
    const response = await authenticatedFetch(`/api/v1/events/${eventId}/contents?content_type=review`);
    const result = await response.json();
    return result.data || [];
  } catch (error) {
    console.error('❌ get event recap failed:', error);
    throw error;
  }
}
...
export async function getEventRatings(eventId: string) {
  try {
    const response = await authenticatedFetch(`/api/v1/events/${eventId}/ratings`);
    const result = await response.json();
    return result.data || [];
  } catch (error) {
    console.error('❌ get event ratings failed:', error);
    throw error;
  }
}
 ...
 * 提交活动评分
 */
export async function submitEventRating(eventId: string, score: number, comment?: string, email?: string) {
  try {
    const token = await getAuthToken();
    if (!token) {
      throw new Error('未登录，请先登录');
    ...
    const response = await fetch(`${API_GATEWAY_URL}/api/v1/events/${eventId}/ratings`, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json',
    ...
    console.error('❌ submit event rating failed:', error);
    throw error;
  }
}
 ...
 * 保存活动回顾
 */
export async function saveEventRecap(eventId: string, title: string, content: string) {
  try {
    const token = await getAuthToken();
    if (!token) {
    ...
    const response = await fetch(`${API_GATEWAY_URL}/api/v1/events/${eventId}/contents/review`, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${token}`,
    ...
    if (!response.ok) {
      const error = await response.json().catch(() => ({ detail: 'Failed to save recap' }));
      throw new Error(error.detail || 'Failed to save recap');
    }

    const result = await response.json();
    return result;
  } catch (error) {
    console.error('❌ save event recap failed:', error);
    throw error;
  }
}
...
export async function uploadGalleryPhoto(eventId: string, file?: File, imageUrl?: string) {
  try {
    ...
    const url = `${API_GATEWAY_URL}/api/v1/events/${eventId}/photos`;
    const response = await fetch(url, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${token}`,
...
export async function getSubscriptionPlans() {
  try {
    const response = await authenticatedFetch('/api/v1/subscriptions/plans');
    const result = await response.json();
    return result.plans || [];
  } catch (error) {
    console.error('❌ getSubscriptionPlans 失败:', error);
    throw error;
  }
}
...
export async function getMySubscription() {
  try {
    const response = await authenticatedFetch('/api/v1/subscriptions/me');
    const result = await response.json();
    return result;
  } catch (error) {
 ...
 * 申请支付（生成待支付记录）
 */
export async function requestPayment(planId: string) {
  try {
    const response = await authenticatedFetch(`/api/v1/subscriptions/request-payment?plan_id=${planId}`, {
      method: 'POST',
    });
    const result = await response.json();
    return result;
  } catch (error) {
    console.error('❌ requestPayment 失败:', error);
    throw error;
  }
}
...
export async function getPendingPayments() {
  try {
    const response = await authenticatedFetch('/api/v1/subscriptions/pending-payments');
    const result = await response.json();
    return result.payments || [];
  } catch (error) {
    console.error('❌ getPendingPayments 失败:', error);
    throw error;
  }
}
...
export async function confirmPayment(paymentId: string) {
  try {
    const response = await authenticatedFetch(`/api/v1/subscriptions/confirm-payment/${paymentId}`, {
      method: 'POST',
    });
    const result = await response.json();
    return result;
  } catch (error) {
    console.error('❌ confirmPayment 失败:', error);
    throw error;
  }
}
 ...
 * 创建支付宝支付
 */
export async function createAlipayPayment(planId: string) {
  try {
    const response = await authenticatedFetch(`/api/v1/alipay/create-payment?plan_id=${planId}`, {
      method: 'POST',
    });
    const result = await response.json();
    return result;
  } catch (error) {
    console.error('❌ createAlipayPayment 失败:', error);
    throw error;
  }
}
 ...
 * 查询支付宝支付状态
 */
export async function queryAlipayPayment(paymentId: string) {
  try {
    const response = await authenticatedFetch(`/api/v1/alipay/query/${paymentId}`);
    const result = await response.json();
    return result;
  } catch (error) {
    console.error('❌ queryAlipayPayment 失败:', error);
    throw error;
  }
}