...
const App = () => {
  ...
  // State for item menu (Edit/Delete dropdown)
  const [activeMenuId, setActiveMenuId] = useState(null);

  // Invite Modal State
  const [isInviteModalOpen, setIsInviteModalOpen] = useState(false);
  const [isCopied, setIsCopied] = useState(false);
  const communityUrl = "herenow.com/community/design-hub"; 
  ...
  const [deleteTarget, setDeleteTarget] = useState(null); // { type: 'member'|'event'|'post', id: number, name: string }
  ...
  useEffect(() => {
    const handleMenuClickOutside = (event) => {
      if (!event.target.closest('.item-menu-trigger') && !event.target.closest('.item-menu-dropdown')) {
  ...
  const tabs = [
    ...
    { id: 'discussions', label: 'Wall', icon: LayoutGrid }, 
  ...
  const ConfirmModal = () => {
    ...
    return (
      <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 animate-fade-in">
        <div className="absolute inset-0 bg-slate-900/20 backdrop-blur-sm transition-opacity" onClick={() => setDeleteTarget(null)}></div>
        <div className="relative bg-white rounded-3xl shadow-2xl p-6 w-full max-w-sm border border-slate-100 scale-100">
          <div className="mb-4">
            ...
            <h3 className="text-xl font-bold text-slate-900 mb-1">
              {deleteTarget.type === 'member' ? 'Remove Member?' : 
               deleteTarget.type === 'event' ? 'Delete Event?' : 'Delete Post?'}
            </h3>
          ...
          <div className="flex gap-3">
            <button onClick={() => setDeleteTarget(null)} className="flex-1 px-4 py-3 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl text-sm font-bold transition-colors">Cancel</button>
            <button onClick={executeDelete} className="flex-1 px-4 py-3 bg-red-500 hover:bg-red-600 text-white rounded-xl text-sm font-bold shadow-lg shadow-red-500/30 transition-colors">Delete</button>
          </div>
        </div>
      </div>
    );
  ...
  const InviteModal = () => {
    ...
    return (
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 animate-fade-in">
        <div className="absolute inset-0 bg-slate-900/20 backdrop-blur-sm transition-opacity" onClick={handleCloseModal}></div>
        <div className="relative bg-white rounded-3xl shadow-2xl shadow-slate-300/50 p-6 w-full max-w-md border border-white/60 transform transition-all scale-100">
          <button onClick={handleCloseModal} className="absolute top-4 right-4 p-2 text-slate-400 hover:text-slate-600 hover:bg-slate-50 rounded-full transition-colors"><X size={20} /></button>
          <div className="flex flex-col items-center text-center mb-6 mt-2">
            <div className="w-14 h-14 bg-orange-50 text-[#FF6B3D] rounded-2xl flex items-center justify-center mb-4 shadow-sm"><UserPlus size={28} /></div>
            <h3 className="font-brand text-2xl font-bold text-slate-900">Invite Members</h3>
            <p className="text-slate-500 text-sm mt-2 px-4">Share this link with your friends or community to let them join <strong>Design Thinking Hub</strong>.</p>
          </div>
          <div className="bg-slate-50 border border-slate-200 rounded-xl p-1.5 flex items-center gap-2 mb-6">
             <div className="flex-grow px-3 py-2 text-sm text-slate-600 font-medium truncate select-all">{communityUrl}</div>
             <button onClick={handleCopyLink} className={`flex items-center gap-2 px-4 py-2.5 rounded-lg font-semibold text-sm transition-all duration-200 shadow-sm ${isCopied ? 'bg-green-500 text-white hover:bg-green-600' : 'bg-white text-slate-900 hover:bg-slate-50 border border-slate-200'}`}>{isCopied ? <Check size={16} /> : <Copy size={16} />}{isCopied ? 'Copied!' : 'Copy'}</button>
          </div>
          <div className="text-center"><button onClick={handleCloseModal} className="text-slate-400 text-sm hover:text-slate-600 font-medium transition-colors">Maybe later</button></div>
        </div>
      </div>
    );
  };
  ...
  const CouponModal = () => {
    ...
    return (
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 animate-fade-in">
        <div className="absolute inset-0 bg-slate-900/20 backdrop-blur-sm transition-opacity" onClick={() => setIsCouponModalOpen(false)}></div>
        <div className="relative bg-white rounded-3xl shadow-2xl p-6 w-full max-w-md border border-white/60 scale-100">
           <div className="flex justify-between items-center mb-6">
             ...
             <button onClick={() => setIsCouponModalOpen(false)} className="p-2 hover:bg-slate-50 rounded-full text-slate-400"><X size={20} /></button>
           </div>
             ...
             <div>
               ...
               <div className="flex gap-2">
                  ...
                  <div className="flex bg-slate-50 p-1 rounded-xl border border-slate-200">
                    <button onClick={() => setNewCoupon({...newCoupon, type: 'percent'})} className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-all ${newCoupon.type === 'percent' ? 'bg-white shadow-sm text-slate-900' : 'text-slate-500 hover:text-slate-700'}`}>%</button>
                    <button onClick={() => setNewCoupon({...newCoupon, type: 'fixed'})} className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-all ${newCoupon.type === 'fixed' ? 'bg-white shadow-sm text-slate-900' : 'text-slate-500 hover:text-slate-700'}`}>$</button>
                  </div>
               </div>
             </div>
           </div>

           <button onClick={handleCreateCoupon} className="w-full py-3 bg-[#FF6B3D] hover:bg-[#ff8057] text-white rounded-xl font-bold shadow-lg shadow-orange-500/20 transition-all active:scale-95">Create Coupon</button>
        </div>
      </div>
    );
  }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="relative" ref={dropdownRef}>
            <button onClick={() => setIsDropdownOpen(!isDropdownOpen)} className="flex items-center gap-2 px-4 py-2 bg-[#FF6B3D] hover:bg-[#ff8057] text-white rounded-xl text-sm font-semibold shadow-lg shadow-orange-500/30 transition-all active:scale-95">
              <Plus size={18} strokeWidth={2.5} />
              <span className="hidden sm:inline">Add New</span>
            </button>
            {isDropdownOpen && (
              <div className="absolute top-full mt-2 right-0 w-64 bg-white/90 backdrop-blur-xl rounded-2xl shadow-2xl shadow-slate-400/20 border border-white/60 p-2 z-50 animate-in fade-in slide-in-from-top-2 duration-200 origin-top-right">
        ...
        <div className="relative pl-4 md:pl-8 pb-12 space-y-8 before:absolute before:left-[19px] md:before:left-[35px] before:top-2 before:bottom-0 before:w-0.5 before:bg-slate-100">
          {sortedEvents.map((item, index) => (
            <div key={item.id} className="relative animate-fade-in" style={{ animationDelay: `${index * 0.1}s` }}>
              ...
              {/* Event Card - Removed overflow-hidden from parent, added rounded corners to image */}
              <div className={`bg-white rounded-2xl border ${item.isPinned ? 'border-orange-200 shadow-orange-500/10' : 'border-slate-100'} shadow-sm hover:shadow-md transition-all duration-200 group ${activeMenuId === item.id ? 'z-20 relative' : 'z-0 relative'}`}>
                <div className="flex flex-col md:flex-row h-full md:h-40">
                  {/* Image Column - Added specific rounding classes */}
                  ...
                  <div className="p-5 flex-grow flex flex-col justify-between relative">
                        ...
                        <div className="flex items-start justify-between gap-4">
                          ...
                          <div className="relative">
                            ...
                            {activeMenuId === item.id && (
                              <div className="item-menu-dropdown absolute right-0 top-full mt-1 w-40 bg-white rounded-xl shadow-xl border border-slate-100 overflow-hidden z-50 animate-in fade-in zoom-in-95 duration-100">
                    ...
                    <div className="flex items-center justify-between mt-auto pt-3 border-t border-slate-50">
                      ...
                      <button className="text-sm font-semibold text-slate-600 hover:text-[#FF6B3D] transition-colors">Manage</button>
                    </div>
                  </div>
                </div>
              </div>
  ...
  const MembersView = () => (
    ...
    <div className="animate-fade-in">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <h2 className="font-brand text-2xl font-bold text-slate-900 flex items-center gap-3">Members<span className="w-6 h-6 rounded-full bg-slate-100 text-slate-500 text-xs font-bold flex items-center justify-center">{members.length}</span></h2>
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => setIsInviteModalOpen(true)} className="flex items-center gap-2 px-5 py-2 bg-slate-900 hover:bg-slate-800 text-white rounded-xl text-sm font-semibold shadow-lg shadow-slate-200 transition-all active:scale-95"><UserPlus size={18} /><span>Invite Member</span></button>
        </div>
      </div>
      <div className="flex flex-col sm:flex-row gap-3 mb-6">
        <div className="relative flex-grow group">
          ...
          <input type="text" placeholder="Search members..." className="w-full pl-10 pr-4 py-2.5 bg-white border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-[#FF6B3D] transition-all placeholder:text-slate-400 shadow-sm"/>
        </div>
        <div className="flex gap-2">
      ...
      {members.length > 0 ? (
            ...
            {members.map((member) => (
              <div key={member.id} className="group flex flex-col md:grid md:grid-cols-12 gap-2 md:gap-4 px-6 py-4 hover:bg-slate-50/80 transition-colors items-center relative">
                <div className="flex items-center gap-3 col-span-12 md:col-span-5 w-full">
                  <div className={`w-10 h-10 rounded-full bg-gradient-to-br ${member.avatarGradient} flex items-center justify-center text-white font-bold text-sm shadow-sm ring-2 ring-white`}>{member.name.split(' ').map(n => n[0]).join('')}</div>
                ...
                <div className="col-span-12 md:col-span-2 w-full md:w-auto flex md:block">
                   <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-medium border ${member.role === 'Owner' ? 'bg-orange-50 text-[#FF6B3D] border-orange-100' : 'bg-slate-100 text-slate-600 border-slate-200'}`}>{member.role === 'Owner' && <Shield size={10} />}{member.role}</span>
                </div>
                ...
                <div className="absolute right-4 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity hidden md:flex gap-2">
                    <a href={`mailto:${member.email}`} className="p-2 bg-white text-slate-400 hover:text-slate-600 hover:bg-slate-50 border border-slate-200 rounded-lg shadow-sm transition-all flex items-center justify-center" title={`Send email to ${member.email}`}><MailIcon size={16} /></a>
                    <div className="relative">
                        <button onClick={(e) => toggleItemMenu(member.id, e)} className="p-2 bg-white text-slate-400 hover:text-slate-600 hover:bg-slate-50 border border-slate-200 rounded-lg shadow-sm transition-all flex items-center justify-center"><MoreHorizontal size={16} /></button>
        ...
        <div className="flex flex-col items-center justify-center py-16 px-4 bg-white/40 rounded-3xl border border-white shadow-sm backdrop-blur-sm">
          ...
          <button onClick={() => setIsInviteModalOpen(true)} className="group relative overflow-hidden rounded-2xl px-8 py-3.5 bg-[#FF6B3D] hover:bg-[#ff8057] active:scale-95 text-white font-semibold shadow-lg shadow-orange-500/30 transition-all duration-200 flex items-center gap-2"><UserPlus size={20} strokeWidth={2.5} /><span>Invite Member</span></button>
        </div>
      )}
    </div>
  );
  ...
  const DiscussionsView = () => {
    ...
    <div className="animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-8">
        <div className="relative flex-grow group">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-[#FF6B3D] transition-colors" size={18} />
          <input type="text" placeholder="Search discussions..." className="w-full pl-10 pr-4 py-2.5 bg-white border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-[#FF6B3D] transition-all placeholder:text-slate-400 shadow-sm"/>
        </div>
        <div className="flex gap-2">
      ...
      <div className="space-y-6">
        {sortedDiscussions.map((post) => {
           ...
           return (
             <div key={post.id} className={`bg-white rounded-3xl border ${post.isPinned ? 'border-orange-200 shadow-orange-500/5' : 'border-slate-100'} p-6 shadow-sm hover:shadow-md transition-all`}>
                {/* Header */}
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                         ...
                         {post.isPinned && <span className="flex items-center gap-1 px-2 py-0.5 bg-orange-50 text-[#FF6B3D] text-[10px] font-bold uppercase tracking-wide rounded-full"><Pin size={10} fill="currentColor" /> Pinned</span>}
                      </div>
        ...
        <div className="bg-white border border-slate-200 rounded-3xl overflow-hidden mb-8 shadow-sm">
            ...
            <div className="divide-y divide-slate-100">
                ...
                <div className="flex items-center justify-between p-5">
                    ...
                    <button onClick={() => setCommunitySettings(prev => ({...prev, isPublic: !prev.isPublic}))} className={`relative inline-flex h-7 w-12 items-center rounded-full transition-colors focus:outline-none ${communitySettings.isPublic ? 'bg-[#FF6B3D]' : 'bg-slate-200'}`}>
        ...
        <div className="bg-white border border-slate-200 rounded-3xl overflow-hidden mb-8 shadow-sm">
            ...
            <div className="divide-y divide-slate-100">
                <div className="flex items-center justify-between p-5">
                    ...
                    <button onClick={() => setCommunitySettings(prev => ({...prev, allowJoin: !prev.allowJoin}))} className={`relative inline-flex h-7 w-12 items-center rounded-full transition-colors focus:outline-none ${communitySettings.allowJoin ? 'bg-[#FF6B3D]' : 'bg-slate-200'}`}>
                ...
                <div className="flex items-center justify-between p-5">
                    ...
                    <button onClick={() => setCommunitySettings(prev => ({...prev, requireApproval: !prev.requireApproval}))} className={`relative inline-flex h-7 w-12 items-center rounded-full transition-colors focus:outline-none ${communitySettings.requireApproval ? 'bg-[#FF6B3D]' : 'bg-slate-200'}`}>
        ...
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-8">
            ...
            <div className="hidden md:flex items-center gap-2">
                <button onClick={() => setActiveTab('events')} className={`flex items-center gap-2 px-3 py-2 rounded-lg transition-all font-semibold text-sm ${activeTab === 'events' ? 'bg-slate-100 text-slate-900' : 'text-slate-500 hover:text-slate-900 hover:bg-slate-50'}`}>
                    <Calendar size={18} strokeWidth={2.5} /><span>Events</span>
                </button>
                <button onClick={() => setActiveTab('members')} className={`flex items-center gap-2 px-3 py-2 rounded-lg transition-all font-semibold text-sm ${activeTab === 'members' ? 'bg-slate-100 text-slate-900' : 'text-slate-500 hover:text-slate-900 hover:bg-slate-50'}`}>
                    <Users size={18} strokeWidth={2.5} /><span>Communities</span>
                </button>
                <button onClick={() => setActiveTab('discussions')} className={`flex items-center gap-2 px-3 py-2 rounded-lg transition-all font-semibold text-sm ${activeTab === 'discussions' ? 'bg-slate-100 text-slate-900' : 'text-slate-500 hover:text-slate-900 hover:bg-slate-50'}`}>
                    <Compass size={18} strokeWidth={2.5} /><span>Discover</span>
                </button>
            </div>
          </div>
          <div className="flex items-center">
             <div className="hidden lg:block text-sm font-medium text-slate-400 mr-6">10:12 PM GMT+8</div>
             <button className="hidden md:flex items-center gap-2 px-5 py-2.5 bg-[#FF6B3D] hover:bg-[#ff8057] text-white rounded-full text-sm font-bold shadow-lg shadow-orange-500/30 transition-all active:scale-95 mr-6">
               <Plus size={20} strokeWidth={2.5} /><span>Create</span>
             </button>
             <div className="flex items-center gap-2 mr-4">
                <button className="p-2 hover:bg-slate-100 rounded-full transition-colors text-slate-500 hover:text-slate-700"><Search size={22} strokeWidth={2} /></button>
                <button className="p-2 hover:bg-slate-100 rounded-full transition-colors text-slate-500 hover:text-slate-700 relative"><Bell size={22} strokeWidth={2} /></button>
             </div>
             <div className="w-10 h-10 rounded-full border-2 border-[#8B5CF6] p-0.5 cursor-pointer hover:shadow-md transition-all">
                <div className="w-full h-full rounded-full bg-slate-100 flex items-center justify-center text-slate-400"><User size={20} /></div>
             </div>
          </div>
        </div>
      </nav>
