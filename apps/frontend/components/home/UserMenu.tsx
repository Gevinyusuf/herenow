...
export default function UserMenu({ onViewChange, onSignOut }: UserMenuProps) {
  ...
  const [avatarError, setAvatarError] = useState(false); // 跟踪头像图片加载失败
  ...
  return (
    <div className="relative" ref={menuRef}>
      <button 
        ...
        className="w-10 h-10 rounded-full flex items-center justify-center text-[#8B5CF6] hover:opacity-80 transition-all active:scale-95 overflow-hidden"
      >
        {user ? (
