                      ...
                      <button
                        onClick={async () => {
                          ...
                          try {
                            const result = await generateAIContent(
                              'planning',
                              `Plan an event with the following details:
Event Type: ${planType}
Target Audience: ${planAudience}
Desired Vibe: ${planVibe}

Please provide a detailed event planning suggestion including:
1. Event concept and highlight moments
2. Suggested activities/agenda flow
3. Interactive elements and engagement strategies
4. Practical execution tips`,
                              { eventType: planType, audience: planAudience, vibe: planVibe }
                            );
                            setPlanIdea(result.data || `Plan for ${planType}: Aim at ${planAudience} with a ${planVibe} vibe. Highlight a signature moment to delight guests.`);
                          } catch (error: any) {
                        ...
                        disabled={isLoading}
                        className="w-full py-3 rounded-xl bg-gradient-to-r from-[#FF6B3D] to-[#FF8E53] text-white font-bold text-sm shadow-md hover:opacity-90 transition disabled:opacity-50 flex items-center justify-center gap-2"
                      >
                        {isLoading ? (
                          <>
